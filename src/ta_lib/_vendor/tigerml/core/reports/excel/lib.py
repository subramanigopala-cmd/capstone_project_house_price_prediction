...
def create_excel_report(
    ...
    contents = process_report_content(
        contents, index_sheet=True, have_plot=have_plot, n_rows=n_rows
    )
    if split_sheets:
        ...
        if isinstance(flat_dict[key_], str):
            ...
            for item in flat_dict[key_]:
                if isinstance(item, str):
                    ...
                    if have_plot:
                        processed_list += [
                            {
                                "data": extract_hv_plot_data(item, n_rows=n_rows)
                        ...
                        processed_list += [
                            {
                                "": extract_hv_plot_data(item, n_rows=n_rows)
                                .reset_index(drop=True)
                                .fillna("")
                            }
                        ]
                elif "matplotlib" in str(type(item)):
                    processed_list += [{"data": "", "plot": item}]
                    ...
                    processed_list += [
                        process_report_content(new_item, index_sheet=False)
                    ]
                else:
                    processed_list += [item]
            ...
            for item in flat_dict[key_]:
                if "holoviews" in str(type(item)):
                    ...
                    processed_list += [{"data": item.data, "plot": ""}]
                elif "Table.Table" in str(type(item)):
                    processed_list += [{"data": item.data.reset_index(), "plot": ""}]
                elif isinstance(item, pd.DataFrame):
            ...
            if have_plot:
                flat_dict[key_] = {
                    "data": extract_hv_plot_data(plot, n_rows=n_rows)
                    .reset_index(drop=True)
                    .fillna(""),
                    "plot": plot,
                }
                ...
                flat_dict[key_] = {
                    "": extract_hv_plot_data(plot, n_rows=n_rows)
                    .reset_index(drop=True)
                    .fillna("")
                }
...
def extract_hv_plot_data(plot, n_rows=100):
    ...
    if isinstance(plot.data, pd.DataFrame):
        if plot.__class__.__name__ == "Distribution":
            data = plot.data[[plot.kdims[0].name]]
            if len(data) > n_rows:
            ...
            req_cols = []
            if plot.kdims and plot.kdims[0].name in plot.data.columns:
                req_cols += [plot.kdims[0].name]
            if plot.vdims and plot.vdims[0].name in plot.data.columns:
                req_cols += [plot.vdims[0].name]
        ...
        if plot.__class__.__name__ == "HoloMap":
            ...
            try:
                df_list = []
                for key_ in plot.kdims[0].values:
                    df = extract_hv_plot_data(plot[key_], n_rows=n_rows)
                    df[plot.kdims[0].name] = key_
                    df_list += [df]
                data = pd.concat(df_list)
            except:
                if len(plot.data) == 1:
            ...
            for key_ in plot.data.keys():
                ...
                try:
                    # df[plot.name] = bokeh_plot.title.text
                    ...
                    if bokeh_plot.__class__.__name__ == "Column":
                        # df[plot.name] = bokeh_plot.children[0].text.split('>')[1].split('<')[0]
                        df["Layout"] = (
                            bokeh_plot.children[0].text.split(">")[1].split("<")[0]
                        )
                    else:
                        df["Layout"] = str(key_)
