...
class ExcelComponent(Component):
    ...
    def save_content(
        ...
        if self.content.__module__.startswith("tigerml.core.reports.excel"):
            from tigerml.core.reports.excel import ExcelImage, ExcelText
            ...
            if isinstance(self.content, ExcelText):
                if self.group:
                    ...
                    width = (
                        self.on_above.width
                        if self.on_above
                        else self.on_below.width if self.on_below else None
                    )
                if width:
                    width = int(width)
                self.content.save(worksheet, workbook, top_row, left_col, width=width)
            elif isinstance(self.content, ExcelImage):
    ...
    def save_to(self, worksheet, workbook, excel_writer):
        ...
        self.save_content(
            worksheet, workbook, excel_writer, self.top_row, self.left_col
        )
