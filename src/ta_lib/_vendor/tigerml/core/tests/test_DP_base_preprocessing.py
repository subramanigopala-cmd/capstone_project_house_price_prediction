...
def test_get_numeric_columns(test_df, selected_col):
    ...
    test_df_[new_col_nm] = test_df_.apply(
        lambda x: (
            True if type(x[old_col_nm]) in [int, float, np.int64, np.float64] else False
        ),
        axis=1,
    )
    if (len(test_df_.groupby([new_col_nm], sort=False).count()) == 1) & test_df_[
        new_col_nm
    ][
        0
    ] == True:  # noqa
...
def make_indexed_data(draw, segment_by):
    ...
    def get_sampling_list(col_num, row_size):
        return ["Seg" + str(col_num) + "_Type" + str(i) for i in range(row_size)] + [
            np.NaN,
            None,
