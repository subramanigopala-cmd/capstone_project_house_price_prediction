...
class MonotonicBinning(BaseEstimator, TransformerMixin):
    ...
    def __init__(
        ...
        self.force_bins = (
            force_bins + 1
        )  # to make the total number of bins as specified by user
    ...
    def fit(self, X, y):
        ...
        try:
            fit_X = pd.DataFrame(
                check_array(
                    temp_X, accept_sparse=True, force_all_finite=False, copy=True
                ),
                columns=fit_on_features,
            )
        except:
            raise ValueError(
                "The input feature(s) should be numeric type. Some of the input features \
    ...
    def train(self, X, y):
        ...
        fitted = self.check_cardinality(
            X
        )  # check the unique value and evaluate if fit is needed or not
        ...
        if fitted:  # fit is required
            ...
            while np.abs(r) < 1 and max_bins > 0:
                try:
                    ...
                    r, p = stats.spearmanr(
                        bins_X_grouped.mean().X, bins_X_grouped.mean().y
                    )  # spearman operation
                    max_bins = max_bins - 1
            ...
            if len(bins_X_grouped) == 1:
                bins = X.quantile(
                    np.linspace(0, 1, force_bins)
                )  # creates a new binnning based on forced bins
    ...
    def transform(self, X, y=None):
        ...
        if not transform_features:
            raise ValueError(
                "Empty list for monotonic transformation. \
                            Estimator has to be fitted to make monotonic transformations"
            )
        ...
        for i in transform_features:
            ...
            if self.custom_binning:
                if original_column_name in self.custom_binning:
                    try:
                        self.bins[original_column_name] = self.custom_binning[
