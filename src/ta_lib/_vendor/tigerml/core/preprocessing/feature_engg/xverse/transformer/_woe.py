...
class WOE(BaseEstimator, TransformerMixin):
    ...
    def __init__(
        ...
        mono_feature_names="all",
        mono_max_bins=20,
        mono_force_bins=3,
        mono_cardinality_cutoff=5,
        mono_prefix=None,
        mono_custom_binning=None,
    ):
        ...
        self.mono_custom_binning = (
            mono_custom_binning  # only used for monotonic transformations
        )
    ...
    def check_datatype(self, X):
        ...
        if not isinstance(X, pd.DataFrame):
            raise ValueError(
                "The input data must be pandas dataframe. But the input provided is "
                + str(type(X))
            )
        return self
    ...
    def fit(self, X, y):
        ...
        if X.shape[0] != y.shape[0]:
            raise ValueError(
                "Mismatch in input lengths. Length of X is "
                + str(X.shape[0])
                + " \
                            but length of y is "
                + str(y.shape[0])
        ...
        if self.monotonic_binning:
            self.mono_bin_clf = MonotonicBinning(
                feature_names=self.mono_feature_names,
                max_bins=self.mono_max_bins,
                force_bins=self.mono_force_bins,
                cardinality_cutoff=self.mono_cardinality_cutoff,
                prefix=self.mono_prefix,
                custom_binning=self.mono_custom_binning,
            )
        ...
        if self.exclude_features:
            self.transform_features = list(
                set(self.transform_features) - set(self.exclude_features)
            )

        temp_X = X[self.transform_features]  # subset data only on features to fit
        temp_X = temp_X.astype(
            "object"
        ...
        self.iv_df = pd.DataFrame(
            {
                "Information_Value": self.woe_df.groupby(
                    "Variable_Name"
    ...
    def treat_missing_values(self, X):
        ...
        if self.treat_missing == "separate":
            ...
            raise ValueError(
                "Missing values could be treated with one of these three options - \
                            'separate', 'mode', 'least_frequent'. \
                            The provided option is - "
                + str(self.treat_missing)
            )
    ...
    def train(self, X, y):
        ...
        temp_woe["WOE"] = np.log(
            temp_woe["Event_Distribution"] / temp_woe["Non_Event_Distribution"]
        )
        temp_woe["Information_Value"] = (
            temp_woe["Event_Distribution"] - temp_woe["Non_Event_Distribution"]
        ) * temp_woe["WOE"]
    ...
    def transform(self, X, y=None):
        ...
        if not transform_features:
            raise ValueError(
                "Empty list for WOE transformation. \
                            Estimator has to be fitted to make WOE transformations"
            )
        ...
        for i in transform_features:
            ...
            if not self.woe_bins:
                raise ValueError(
                    "woe_bins variable is not present. \
