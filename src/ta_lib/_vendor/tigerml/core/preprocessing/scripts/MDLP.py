...
class MDLP_Discretizer(BaseEstimator, TransformerMixin):
    def __init__(self, features=None, raw_data_shape=None):
        """Initializes discretizer object.

        initializes discretizer object:
            saves raw copy of data and creates self._data
            with only features to discretize and class
        ...
        :param features: if !None, features that the user
                        wants to discretize specifically
        :return:
        """
        ...
        if (
            features is None
        ):  # Assume all columns are numeric and need to be discretized
            if raw_data_shape is None:
                raise Exception(
                    "If feautes=None, raw_data_shape must be a non-empty tuple"
                )
            ...
            if np.issubdtype(features.dtype, np.integer):
                ...
                if raw_data_shape is None:
                    raise Exception(
                        ...
                        "raw_data_shape must be != None"
                    )
                if len(features) != self._data_raw.shape[1]:
                    raise Exception(
                        "Column boolean mask must be " "of dimensions (NColumns,)"
                    )
                self._col_idx = np.where(features)
            else:
                raise Exception(
                    "features argument must a np.array of "
                    "column indices or a boolean mask"
                )

    def fit(self, X, y):
        self._data_raw = X  # copy of original input data
        self._class_labels = y.reshape(
            -1, 1
        )  # make sure class labels is a column vector
        self._classes = np.unique(self._class_labels)

        if (
            len(self._col_idx) != self._data_raw.shape[1]
        ):  # some columns will not be discretized
            self._ignore_col_idx = np.array(
                [
                    var
                    for var in range(self._data_raw.shape[1])
                    if var not in self._col_idx
                ]
            )
    ...
    def MDLPC_criterion(self, X, y, feature_idx, cut_point):
        ...
        :param partition_index: index of the sample (dataframe partition)
                                in the interval of interest
        :return: True/False, whether to accept the partition
        """
        # get dataframe only with desired attribute and class columns,
        # and split by cut_point
    ...
    def feature_boundary_points(self, values):
        ...
        :param feature: feature of interest
        :param partition_index: indices of rows for which feature
                                value falls whithin interval of interest
        :return: array with potential cut_points
        """
        missing_mask = np.isnan(values)
        data_partition = np.concatenate(
            [values[:, np.newaxis], self._class_labels], axis=1
        )
        ...
        unique_vals = np.unique(
            data_partition[:, 0]
        )  # each of this could be a bin boundary
        # Find if when feature changes there are different class values
        boundaries = []
        for i in range(
            1, unique_vals.size
        ):  # By definition first unique value cannot be a boundary
            previous_val_idx = np.where(data_partition[:, 0] == unique_vals[i - 1])[0]
            current_val_idx = np.where(data_partition[:, 0] == unique_vals[i])[0]
            merged_classes = np.union1d(
                data_partition[previous_val_idx, 1], data_partition[current_val_idx, 1]
            )
            if merged_classes.size > 1:
                boundaries += [unique_vals[i]]
        boundaries_offset = np.array(
            [previous_item(unique_vals, var) for var in boundaries]
        )
    ...
    def compute_boundary_points_all_features(self):
        ...
        Computes all possible boundary points for each
        attribute in self._features (features to discretize)
        :return:
        """
        ...
        boundaries[:, self._col_idx] = np.apply_along_axis(
            padded_cutpoints_array,
            0,
    ...
    def boundaries_in_partition(self, X, feature_idx):
        ...
        find cut points that fall within a feature-partition's
        ...
        mask = np.logical_and(
            (self._boundaries[:, feature_idx] > range_min),
    ...
    def best_cut_point(self, X, y, feature_idx):
        """Selects the best cut point for a feature in a data partition.

        based on information gain
        :param data: data partition (pandas dataframe)
        :param feature: target attribute
        :return: value of cut point with highest information gain
                (if many, picks first). None if no candidates
        """
        candidates = self.boundaries_in_partition(X, feature_idx=feature_idx)
        if candidates.size == 0:
            return None
        gains = [
            (cut, cut_point_information_gain_numpy(X, y, cut_point=cut))
    ...
    def single_feature_accepted_cutpoints(self, X, y, feature_idx):
        ...
        :param feature: attribute of interest
        :param partition_index: index of examples in data partition for
                                which cuts are required
        :return: list of cuts for binning feature in partition covered
                 by partition_index
        """
        # Delte missing data
        mask = np.isnan(X)
        ...
        if decision:
            ...
            self.single_feature_accepted_cutpoints(
                left_partition, y[left_mask], feature_idx
            )
            self.single_feature_accepted_cutpoints(
                right_partition, y[right_mask], feature_idx
            )
    ...
    def all_features_accepted_cutpoints(self):
        ...
        for attr in self._col_idx:
            self.single_feature_accepted_cutpoints(
                X=self._data_raw[:, attr], y=self._class_labels, feature_idx=attr
            )
        return
    ...
    def generate_bin_descriptions(self):
        ...
        Saves a new, discretized file, and a description of
        the bins
    ...
    def apply_cutpoints(self, data):
        ...
        Saves a new, discretized file, and a description of the bins
        :param out_data_path: path to save discretized data
        :param out_bins_path: path to save bins description
        :return:
        """
        for attr in self._col_idx:
            if len(self._cuts[attr]) == 0:
                ...
                cuts = [-np.inf] + self._cuts[attr] + [np.inf]
                discretized_col = (
                    np.digitize(x=data[:, attr], bins=cuts, right=False).astype("float")
                    - 1
                )
