class Outlier:
    ...
    def transform(self, X, drop=False):
        ...
        for col, lb in self.lb.items():
            lb_casted = df[col].dtype.type(
                lb
            )  # typecastted to have the same data type as the column
            fil_lower = df[col] < lb_casted
            if drop is True:
                df = df[~fil_lower]
            else:
                df.loc[fil_lower, col] = lb_casted
        for col, ub in self.ub.items():
            ub_casted = df[col].dtype.type(
                ub
            )  # typecastted to have the same data type as the column
            fil_upper = df[col] > ub_casted
            if drop is True:
                df = df[~fil_upper]
            else:
                df.loc[fil_upper, col] = ub_casted
        return df
