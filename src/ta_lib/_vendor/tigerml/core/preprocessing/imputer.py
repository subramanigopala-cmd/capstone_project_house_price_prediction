...
class Imputer(_BaseImputer, TransformerMixin):
    ...
    def fit(self, data, y=None):
        ...
        for col in self.imputed_data_.columns:
            if compute_if_dask(
                ...
                _LOGGER.info(
                    "All the values of {} are missing. Dropping the column.".format(col)
                )
    ...
    def transform(self, data):
        ...
        # handled numpy array input for imputer class
        # assigning and reassignned n_features_in_ before and after validate data to match the n_features_in_ in both fit and transform
        if isinstance(data, pd.DataFrame) or isinstance(data, td.DataFrame):
            self.features_ = data_without_datetime.columns.tolist()
            self.n_features_in_ = len(self.features_)

        # data_without_datetime = data

        data_without_datetime = validate_data(
            self,
            data_without_datetime,
            accept_sparse=False,
            dtype=object,
            ensure_all_finite="allow-nan",
            reset=False,  # Do not reset n_features_in_ in transform
        )
        ...
        if data.shape[1] != self.n_features_in_:
            raise ValueError(
                ...
                "from the number of features {} in fit.".format(
                    data.shape[1], self.n_features_in_
                )
            )
        data_ = data.copy()
        ...
        for col in cols:
            if col not in self.drop_cols_:
                ...
                imputed_data_[col] = (
                    self.imputer_[col]
                    .transform(data_[col].to_numpy().reshape(-1, 1))
                    .ravel()
                )
                self.imputation_summary_.update(
                    {
