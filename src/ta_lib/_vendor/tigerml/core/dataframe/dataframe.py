...
class DataFrame(BackendMixin):
    ...
    def describe(self, *args, **kwargs):
        ...
        if self.backend == BACKENDS.dask:
            return self._data.describe(*args, **kwargs).compute()
    ...
    def convert_datetimes(self, format_dict=None, **pandas_kwargs):
        ...
        if not format_dict:
            ...
            # def guess_datetime_format_for_array_custom(arr, **kwargs):
            ...
            def guess_date_format(date_array):
                date_list = (
                    date_array.tolist()
                    if isinstance(date_array, np.ndarray)
                    else date_array
                )
                ...
                votes = {"%d-%m-%Y": 0, "%m-%d-%Y": 0}

                for timestr in date_list:
                    match = re.match(pattern, timestr)
                    if match:
                        part1, part2 = map(int, match.groups()[:2])
                        if part1 > 12:
                            votes["%d-%m-%Y"] += 1
                        elif part2 > 12:
                            votes["%m-%d-%Y"] += 1
                ...
                if isinstance(timestr, str):
                    try:
                        ...
                        if "could not convert" in str(float_conversion_error):
                            try:
                                parse(timestr)
                                return 1, guess_date_format(np.array([timestr]))
                            # except (TypeError, ValueError) as parsing_error:
                                ...
                                if any(
                                    ...
                                    _LOGGER.warning(f"{parsing_error}")
                                    # giving a warning message instead of raising an exception
                                    return 0, None
                        else:
                            _LOGGER.debug(
    ...
    def groupby(self, *args, **kwargs):
        ...
        # if self.backend == BACKENDS.dask:
        return TigerWrapper(
            self._data.groupby(*args, observed=kwargs.pop("observed", False), **kwargs)
        )
