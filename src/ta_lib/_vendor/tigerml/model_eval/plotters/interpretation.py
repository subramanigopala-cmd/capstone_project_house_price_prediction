...
def get_shap_summary_plot(
    ...
    if native_plot:
        ...
        mean_abs_shap_values = np.mean(np.abs(shap_values_matrix), axis=2)
...
class ModelInterpretation:
    ...
    def get_errorbucket_profiles(self):
        ...
        if X["errorbucket_label"].isnull().sum() > 0:
            ...
            X = X.loc[X["errorbucket_label"].isnull() == False,]  # noqa: E712
        if self.has_test:
    ...
    def _get_shap_values_plot(self, X, features):
        ...
        if features is not None:
            ...
            if shap_values.ndim == 3:
                shap_values = shap_values[:, cols_index, 0]
            else:
    ...
    def get_plots(
        ...
        if "feature_importance" in plots_dict.keys():
            if "from_model" in plots_dict["feature_importance"].keys():
                plots_dict["feature_importance"]["from_model"] = (
        ...
        if "error_analysis" in plots_dict.keys():
            ...
            if "from_shap" in plots_dict["error_analysis"].keys():
                plots_dict["error_analysis"]["from_shap"] = (
        ...
        if "shap_interpretation" in plots_dict.keys():
            ...
            if shap_values.ndim == 3:
                shap_values_var = shap_values[:, :, 0]
            else:
                shap_values_var = shap_values

            plots_dict["shap_interpretation"]["dependence_plots"] = (
                self.get_dependence_plots(X, shap_values=shap_values_var, features=cols)
            )
