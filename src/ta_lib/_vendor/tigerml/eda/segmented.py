...
class SegmentedEDAReport(
    ...
    def _segment_cleaner(self, data=None):
        ...
        for col in self.segment_by:
            df.loc[is_missing(df[col], self.NA_VALUES), col] = (
                None  # chained indexing is replaced by .iloc
            )
    ...
    def outliers_per_segment(self, group_segments_by=None):
        ...
        for i in unique:
            ...
            if isinstance(idf._data.values[0], str):
        ...
        outliers_sum = outliers_df.select_dtypes(include=np.number).sum(axis=1)
    ...
    def numeric_summary(self, feature=None, group_segments_by=None, quick=True):
        ...
        if feature:
            ...
            if not quick:
                if not isinstance(summary_dict["Complete Data"], str):
                    for groupby_col in group_segments_by:
                        segmented_summary = {}
                        summary_dict[f"{groupby_col}_wise"] = segmented_summary
                        cols = [
    ...
    def non_numeric_summary(self, feature=None, group_segments_by=None, quick=True):
        ...
        if feature:
            ...
            if not quick:
                ...
                if (
                    not isinstance(summary_dict["Complete Data"], str)
                    and len(non_num_cols) > 0
                ):
                    for groupby_col in group_segments_by:
                        segmented_summary = {}
                        summary_dict[f"{groupby_col}_wise"] = segmented_summary
    ...
    def _get_segment_contribution(
        ...
        cum_name = f"{metric_aggregation.__name__} of {metric_column}"  # the sum function object  is replaced with "sum" to be called for groupby
        metric_aggregation = "sum" if metric_aggregation is sum else metric_aggregation
        contributions = (
        ...
        if len(group_segments_by) > 1 or len(contributions) > 30 or bin_limits:
            ...
            last_row_index = contributions.index[-1]
            contributions.loc[last_row_index, cum_perc_name] = (
