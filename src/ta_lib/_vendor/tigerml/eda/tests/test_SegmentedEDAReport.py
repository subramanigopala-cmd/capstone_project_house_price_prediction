...
def make_df_with_select_cols(draw, non_seg_cols, non_empty_row="have"):
    ...
    if non_empty_row:
        ...
        for i in range(3):
            ...
            df = pd.concat(
                [df, pd.DataFrame([new_series], columns=df.columns)], ignore_index=True
            )
    return df
...
def df_with_select_col_and_segments(
    ...
    if seg_has_na:
        if have_segments == 1:
            ...
            data_df_all.loc[ind, "Lev_1"] = na_val
        elif have_segments == 2:
            ind = draw(generate_index)
            data_df_all.loc[ind, "Lev_1"] = na_val
            ind = draw(generate_index)
            data_df_all.loc[ind, "Lev_2"] = na_val
        elif have_segments == 3:
            ind = draw(generate_index)
            data_df_all.loc[ind, "Lev_1"] = na_val
            ind = draw(generate_index)
            data_df_all.loc[ind, "Lev_2"] = na_val
            ind = draw(generate_index)
            data_df_all.loc[ind, "Lev_3"] = na_val
...
def test_numeric_summary(
    ...
    if test_type == "T1":
        ...
        if not isinstance(calculated_dict["Complete Data"], str) and not quick_state:
        ...
        if quick_state:
            ...
            expected_dict_keys = ["Complete Data"]
            if not isinstance(calculated_dict["Complete Data"], str):
...
def test_get_segment_contribution(
    ...
    if test_type == "T1":
        ...
        assert isinstance(computed_plot, holoviews.core.overlay.Overlay)
