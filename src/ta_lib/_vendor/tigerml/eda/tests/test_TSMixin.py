...
def make_ts(draw, start_date=None, have_miss_dups=False, shuffle_ts="sorted_ts"):
    ...
    exp = (
        "pd.date_range(start='{}', periods={}, freq=timedelta({}={}))"
    ...
    if have_miss_dups:
        ...
        ts_df.loc[nat_index, "ts_col"] = pd.NaT
...
def df_with_ts_col(
    ...
    if not multi_period:
        ...
        data_df_all = pd.concat(data_df_list, ignore_index=True)
    ...
    if have_miss_dups:
        if have_segments == 0:
            ...
            if isinstance(computed_time_repetitions, dict):
                computed_time_repetitions = computed_time_repetitions[
                    "no_of_repetitions"
                ]
