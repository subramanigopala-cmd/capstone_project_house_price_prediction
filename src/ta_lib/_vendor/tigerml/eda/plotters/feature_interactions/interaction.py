...
def get_bivariate_plot(df, dtypes, corr_df, x_var, y_var):
    ...
    for col in [x_var, y_var]:
        if dtypes[col] == "cat" and data[col].nunique() > MAX_CAT_LEVELS:
            ...
            data.loc[~detigerify(data[col].isin(top_levels)), col] = (
                others_placeholder  # chained indexing is replaced with loc to fix setwithcopy warning
            )
