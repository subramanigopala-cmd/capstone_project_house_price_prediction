...
from tigerml.pyspark.model_eval.interpretation import get_shap_values
...
def get_spark_data(
    spark, df, y_col="label", classification=False, train_prop=0.7, stratify=True
):
...
def get_testing_data(draw, is_classification, spark):
    ...
    train_df, test_df = get_spark_data(
        spark, df, classification=is_classification, train_prop=train_prop
    )
    return train_df, test_df
...
@pytest.mark.parametrize("n_partitions", ["n_partitions1", "n_partitions2"])
@pytest.mark.parametrize("n_threads", ["n_threads2", "n_threads3", "n_threads4"])
...
def test_get_binary_classification_metrics(test_df, n_threads, n_partitions, test_type):
    ...
    test_sdf = pandas_to_spark(spark_session, test_df)
    spark_session.conf.set("spark.sql.shuffle.partitions", int(n_partitions[-1]))
    results = get_binary_classification_metrics(
        spark_session, test_sdf, "y_col", y_pred_col="y_pred_col"
    )
    if test_type == "T1":
...
@pytest.mark.parametrize("n_partitions", ["n_partitions1", "n_partitions2"])
@pytest.mark.parametrize("n_threads", ["n_threads2", "n_threads3", "n_threads4"])
@pytest.mark.parametrize("report_type", [1])
@settings(max_examples=10, deadline=None)
@given(test_df=data())
def test_get_regression_report(test_df, n_threads, n_partitions, report_type):
    ...
    df_train, df_test = test_df.draw(
        get_testing_data(is_classification=False, spark=spark_session)
    )
    if report_type == 1:
        reg_obj1 = RegressionReport(
            spark=spark_session,
            train_df=df_train,
            label_Col="Target",
...
def test_get_classification_report(test_df, n_threads, n_partitions, report_type):
    ...
    df_train, df_test = test_df.draw(
        get_testing_data(is_classification=True, spark=spark_session)
    )
    if report_type == 1:
        cls_obj1 = ClassificationReport(
            spark=spark_session,
            train_df=df_train,
            label_Col="Target",
...
@given(test_df=data())
def test_get_shap_values(test_df):
    # Create a SparkSession object
    spark = (
        SparkSession.builder.master("local")
        .appName("TigerModelEvalSession")
        .getOrCreate()
    )
    # Create a Spark DataFrame for testing
    df_train, df_test = test_df.draw(
        get_testing_data(is_classification=True, spark=spark)
    )
    # Define the feature columns
    target_var = "label"
    feature_cols = [col for col in df_train.columns if col not in [target_var]]
    class_assembler = VectorAssembler(inputCols=feature_cols, outputCol="features")
    class_df_train = class_assembler.transform(df_train)
    class_df_test = class_assembler.transform(df_test)

    # Test case 1: Testing SHAP values for a linear regression model
    lr_class = LogisticRegression(
        featuresCol="features", labelCol=target_var, predictionCol="prediction"
    )
    class_model = lr_class.fit(class_df_train)
    class_df_train1 = class_model.transform(class_df_train)
    class_df_test1 = class_model.transform(class_df_test)
    out_df = get_shap_values(class_model, class_df_train1, feature_cols, spark)
    assert (
        out_df.count() == class_df_train1.count()
    )  # Check the number of rows in the output DataFrame

    # Test case 2: Testing SHAP values for a tree-based model
    rfc_class = RandomForestClassifier(
        featuresCol="features", labelCol=target_var, predictionCol="prediction"
    )
    class_model = rfc_class.fit(class_df_train)
    class_df_train2 = class_model.transform(class_df_train)
    class_df_test2 = class_model.transform(class_df_test)
    out_df = get_shap_values(class_model, class_df_train2, feature_cols, spark)
    assert (
        out_df.count() == class_df_train2.count()
    )  # Check the number of rows in the output DataFrame

    # Create a Spark DataFrame for testing
    df_train, df_test = test_df.draw(
        get_testing_data(is_classification=False, spark=spark)
    )
    # Define the feature columns
    target_var = "label"
    feature_cols = [col for col in df_train.columns if col not in [target_var]]
    reg_assembler = VectorAssembler(inputCols=feature_cols, outputCol="features")
    reg_df_train = reg_assembler.transform(df_train)
    reg_df_test = reg_assembler.transform(df_test)

    # Test case 1: Testing SHAP values for a linear classification model
    lr_class = LinearRegression(
        featuresCol="features", labelCol=target_var, predictionCol="prediction"
    )
    reg_model = lr_class.fit(reg_df_train)
    reg_df_train1 = reg_model.transform(reg_df_train)
    reg_df_test1 = reg_model.transform(reg_df_test)
    out_df = get_shap_values(reg_model, reg_df_train1, feature_cols, spark)
    assert (
        out_df.count() == reg_df_train1.count()
    )  # Check the number of rows in the output DataFrame

    # Test case 1: Testing SHAP values for a linear classification model
    lr_class = RandomForestRegressor(
        featuresCol="features", labelCol=target_var, predictionCol="prediction"
    )
    reg_model = lr_class.fit(reg_df_train)
    reg_df_train2 = reg_model.transform(reg_df_train)
    reg_df_test2 = reg_model.transform(reg_df_test)
    out_df = get_shap_values(reg_model, reg_df_train2, feature_cols, spark)
    assert (
        out_df.count() == reg_df_train2.count()
    )  # Check the number of rows in the output DataFrame
