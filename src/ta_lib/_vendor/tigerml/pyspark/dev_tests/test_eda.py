...
def generate_data_paths():
    ...
    # download housing dataset using python-wget
    housing_url = "https://raw.githubusercontent.com/ageron/data/refs/heads/main/housing/housing.csv"
    housing_path = os.path.join(directory, "housing.csv")
    if not os.path.exists(housing_path):
        wget.download(housing_url, out=housing_path)
...
def test_get_outliers_table(value, n_threads, n_partitions, test_type, get_index):
    ...
    test_sdf_cleaned = clean_columns(test_sdf)
    spark_session.conf.set("spark.sql.shuffle.partitions", int(n_partitions[-1]))
    results = get_outliers_table(test_sdf_cleaned)
    if test_type == "T2":
...
def test_describe_data(value, n_threads, n_partitions, test_type):
    ...
    test_sdf_cleaned = clean_columns(test_sdf)
    spark_session.conf.set("spark.sql.shuffle.partitions", int(n_partitions[-1]))
    results = describe_data(
        test_sdf_cleaned, columns=list_numerical_columns(test_sdf_cleaned)
    )
...
def test_feature_analysis_table(value, n_threads, n_partitions, test_type):
    ...
    test_sdf_cleaned = clean_columns(test_sdf)
    spark_session.conf.set("spark.sql.shuffle.partitions", int(n_partitions[-1]))
    numerical_description, categorical_description = feature_analysis_table(
        test_sdf_cleaned
    )
    if test_type == "T2":
        assert (
...
def test_density_plots_numerical(value, n_threads, n_partitions):
    ...
    test_sdf_cleaned = clean_columns(test_sdf)
    spark_session.conf.set("spark.sql.shuffle.partitions", int(n_partitions[-1]))
    result = density_plots_numerical(test_sdf_cleaned)
...
def test_feature_analysis_pca(value, n_threads, n_partitions):
    ...
    result = feature_analysis_pca(test_sdf_cleaned, num_cols[0])
...
def test_key_drivers(value, n_threads, n_partitions, is_classification):
    ...
    if is_classification:
        ...
        if len(cat_col) > 0:
            result = EDAReportPyspark(
                test_sdf_cleaned, is_classification=True, y=cat_col[0]
            )
            result.key_drivers(
                y=cat_col[0],
                save_as=".html",
        ...
        if len(num_cols) > 0:
            result = EDAReportPyspark(
                test_sdf_cleaned, is_classification=False, y=num_cols[0]
            )
            result.key_drivers(
                y=num_cols[0],
                save_as=".html",
...
def test_data_health_recommendations(value, n_threads, n_partitions):
    ...
    test_sdf_cleaned = clean_columns(test_sdf)
    spark_session.conf.set("spark.sql.shuffle.partitions", int(n_partitions[-1]))
    result = data_health_recommendations(test_sdf_cleaned)
    assert isinstance(result, pd.core.frame.DataFrame) and result.columns.tolist() == [
        "Recommendations",
