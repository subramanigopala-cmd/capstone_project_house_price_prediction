...
from .dp import list_categorical_columns, list_numerical_columns
...
def encode_categorical_features(data, cols=[], rules=dict(), prefix=""):
    ...
    data : pyspark.sql.DataFrame
        The input Spark DataFrame containing the dataset.
    cols : list, optional
        List of columns to encode. If not provided, considers all categorical columns in the data.
        Defaults to an empty list.
    rules : dict(col_name: rule_dict)
        A dictionary where keys are column names and values are rule_dicts.
        A rule_dict contains the encoding method and its relevant arguments.
        ...
        .. code-block:: python
            ...
            # Ordinal encoding with a custom mapper
            {'method': 'ordinal', 'mapper': {<class>: <value_to_be_encoded>}}

            # Target encoding (e.g., mean of target_col)
            {'method': 'target', 'target_col': <target_column_name>, 'metric': <one of mean, median>}

        Defaults to an empty dictionary.
    prefix : str, optional
        Prefix to add to the encoded column, resulting in `{prefix}_{col_name}`.
        Defaults to `{method}_encoded_{col_name}`.
...
class Encoder(Estimator, Transformer):
    ...
    ----------
    cols : list, optional
        List of columns to encode. By default, considers all categorical variables in the data.
    rules : dict, optional
        Dictionary containing encoding rules for each column. Default is an empty dictionary.
    prefix : str, optional
        Prefix to add to the encoded column names. Default is an empty string.

    Examples
    --------
    >>> encoder = Encoder(cols=['category'], rules={'category': {'method': 'onehot'}}, prefix='encoded_')
    >>> encoder.fit(train_data)
    >>> transformed_data = encoder.transform(test_data)
    """
...
class Binner(Estimator, Transformer):
    """Transformer for Binning methods.
    ...
    rules : dict(col_name: rule_dict)
        A dictionary where keys are column names and values are rule_dicts.
        A rule_dict specifies the type of binning (e.g., Quantile or by values)
        and the relevant arguments for the chosen method.
        ...
        .. code-block:: python
            ...
            {
                'median_house_value': {
                    'n': <int>,  # n equal quantile buckets
                    'value': <array of numbers (int/float)>,
                            # Buckets produced based on provided values
                    'size': <int/float>,  # Size of each interval/bucket
                    'left': <boolean, default: True>
                            # If True, bucket size counting starts from left;
...
class FeatureEliminator:  # Estimator,Transformer
    ...
    target_type : str, optional
        Type of the target variable, either 'continuous' or 'categorical'. Default is 'continuous'.

    Examples
    --------
    >>> eliminator = FeatureEliminator(spark, dataset, target_col='target', target_type='continuous')
    >>> result = eliminator.transform(method='sparseness', threshold=0.1)
    """
...
def feature_elimination_by_sparseness(data, threshold=0.1):
    ...
    threshold : float, optional
        A number between 0 and 1. Features with zero values density greater than or equal to
        the threshold will be returned. Default is 0.1.
...
def feature_elimination_by_cv(data, threshold=0.001):
    ...
    ----------
    data : pyspark.sql.dataframe.DataFrame
        The Data frame to eliminate features
    threshold : float, optional
        Features with a coefficient of variation (cv) less than or equal to the threshold will be returned.
        Default is 0.001.
...
def feature_elimination_by_missing_values(data, threshold=0.1):
    ...
    data : pyspark.sql.dataframe.DataFrame
        The Data frame to eliminate features
    threshold : float, optional
        A number between 0 and 1. Features with missing values density greater than or equal to
        the threshold will be returned. Default is 0.1.
...
def feature_elimination_by_correlation(data, target_col="target", threshold=0.1):
    ...
    threshold : float, optional
        A number between 0 and 1. Features with the absolute Pearson's correlation coefficient less than
        or equal to the threshold will be returned. Default is 0.1.
...
def feature_elimination_by_mutual_value(
    ...
    data : pyspark.sql.dataframe.DataFrame
        The input DataFrame containing the dataset.
    threshold : float, optional
        A number between 0 and 1. Features with mutual information less than or equal to
        the threshold will be returned. Default is 0.1.
    target_col : str, optional
        The name of the target column. Default is "response".
...
def feature_elimination_by_lasso(
    ...
    """Eliminate Features based on lasso/linear/logistic regression.
    ...
    target_col : str, optional
        The name of the target column. Default is "target".
    target_type : str, optional
        Type of the target variable. Choose from "binary_categorical" or "continuous". Default is "continuous".
    alpha : float, optional
        Regularization strength. A number between 0 and 1. Default is 1.
