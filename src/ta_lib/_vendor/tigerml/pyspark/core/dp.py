...
def clean_columns(data, strip=True, lower=False, sep="_"):
    ...
    data : pyspark.sql.DataFrame
        The input Spark DataFrame to clean the columns.
    strip : bool, optional
        If True, remove trailing whitespaces from column names. Default is True.
    lower : bool, optional
        If True, convert the column names to lowercase. Default is False.
    sep : str, optional
        The separator to use when converting column names. Only '_', '-', and space are allowed.
        Default is "_".
...
def _clean_string_val(
    ...
    for col_ in cols:
        if strip:
            ...
            df = df.withColumn(col_, F.regexp_replace(F.col(col_), r"\s+", " "))  # noqa
        ...
        if remove_chars_in_braces:
            ...
            df = df.withColumn(
                col_,
                F.regexp_replace(F.col(col_), r"\(.*\)|\[.*\]", ""),  # noqa
            )  # noqa
        else:
...
def list_numerical_categorical_columns(data, threshold=5):
    ...
    threshold: int, optional
        The threshold percentage of unique values for considering a column as numerical categorical.
        Default is 5.

    Returns
    -------
    numeric_cat_cols: list
            List of column names identified as numerical categorical.
    """
    schema = data.dtypes
...
def list_numerical_columns(data):
    ...
    -------
    numerical_cols: list
            list of column names with numerical values
    """
    schema = data.dtypes
    numerical_cols = [
...
def list_datelike_columns(data):
    ...
    -------
    d_cols: list
        List of column names identified as datelike (date or timestamp).
    """
    schema = data.dtypes
...
def identify_col_data_type(data, col):
    ...
    col : str
        The name of the column whose data type needs to be identified.

    Returns
    -------
    str
        The identified data type of the column. Possible values: "boolean", "numerical", "date_like", "categorical".
    """
    num_cols = list(
...
def _calculate_outlier_bounds_iqr(data, cols, iqr_multiplier=1.5):
    ...
    temp = data.approxQuantile(cols, [0.25, 0.75], relativeError=0.0001)
...
def identify_missing_values(data):
    ...
    -------
    df : pyspark.sql.DataFrame
        DataFrame with columns representing the count of missing values for each column.
    """
    try:
        df = data.select(
            [
...
def test_train_split(
    ...
    train_prop : float
        The proportion (0-1) of the training data.
    random_seed : int, optional
        The seed used for random number generation. Default is 42.
    stratify : bool, optional
        If True, the data is split with respect to the class in the target_col. Default is False.
    target_type : str
        The type of the target variable. Possible values: "continuous" or "categorical".
        Default is "continuous".

    Returns
    -------
    train, test : pyspark.sql.DataFrame, pyspark.sql.DataFrame
        The training and testing Spark DataFrames.
    """
