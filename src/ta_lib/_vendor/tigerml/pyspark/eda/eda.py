...
def setanalyse(df1, df2, col, simplify=True, exceptions_only=False):
    """
    Perform set analysis on two Spark DataFrames based on a specified column.

    Parameters
    ----------
    df1 : pyspark.sql.DataFrame
        First Spark DataFrame for set analysis.
    df2 : pyspark.sql.DataFrame
        Second Spark DataFrame for set analysis.
    col : str
        The column in the DataFrames to be used for set analysis.
    simplify : bool, default=True
        If True, the output values represent the count of elements in sets.
        If False, the output values are the actual sets.
    exceptions_only : bool, default=False
        If True, returns only the sets A-B and B-A for efficiency.
        If False, returns all four sets: A-B, B-A, A union B, and A intersection B.

    Returns
    -------
    dict
        A dictionary containing the set analysis results based on the specified parameters.
        The keys represent different set operations:

        - 'A-B': Set of elements in df1 but not in df2.

        - 'B-A': Set of elements in df2 but not in df1.

        - 'AuB': Set union of elements in df1 and df2 (only if exceptions_only is False).

        - 'A^B': Set intersection of elements in df1 and df2 (only if exceptions_only is False).
    """
...
def get_outliers(data, cols=None):
    ...
    for column in cols:
        iqr_min, iqr_max = iqr_bounds[column]["min_b"], iqr_bounds[column]["max_b"]
        mean_min, mean_max = mean_bounds[column]["min_b"], mean_bounds[column]["max_b"]

        # Use PySpark's count function to calculate outliers efficiently
        iqr_lower_count = data.filter(col(column) < iqr_min).count()
        iqr_upper_count = data.filter(col(column) > iqr_max).count()
        mean_lower_count = data.filter(col(column) < mean_min).count()
        mean_upper_count = data.filter(col(column) > mean_max).count()

        # Store the counts in dictionaries
        outliers_dict_iqr[column] = iqr_lower_count, iqr_upper_count
        outliers_dict_mean[column] = mean_lower_count, mean_upper_count
...
def get_outliers_table(data):
    ...
    outliers_df: pandas.core.frame.DataFrame
        DataFrame with counts of outliers for both IQR and standard deviation bounds.
...
def data_health_recommendations(data):
    ...
    multi_null_rows = (
        ...
        - data_without_imputation.dropna(thresh=threshold).count()
    )
    ...
    data_without_null_imputation = data_without_nulls.drop(
        *imputation_cols_total
    ).dropna()
    null_rows = (
...
def health_analysis(data, save_as=None, save_path=""):
    ...
    ----------
    data : pyspark.sql.DataFrame
        The input Spark DataFrame for analysis.
    save_as : str, optional
        File format to save the report (e.g., ".html"). If not provided, the report will not be saved.
    save_path : str, optional
        Location where the report should be saved. By default, the report is saved in the working directory.

    Returns
    -------
    health_analysis_report : dict
        A dictionary containing various health analysis outputs.
    ...
    health_analysis_report.update(
        {"data_health_recommendation": data_health_recommendations(data)}
    )
    if save_as:
...
def non_numeric_frequency_plots(data, cols=[]):
    ...
    for col_ in sorted(cols):
        # Summary Table
        summary_df = data.select(col_).describe().toPandas().T
        ...
        freq_df = (
            data.groupBy(col_).count().orderBy(F.desc("count")).limit(20).toPandas()
        )
        if list(freq_df.columns).count("count") > 1:
            freq_df.columns = [
                f"frequency" if col == "count" and i == 1 else col
                for i, col in enumerate(freq_df.columns)
            ]
        freq_plot = hvPlot(freq_df).bar(
            x=col_,
            y="count",
            title=f"Frequency Plot for {col_}",
            invert=True,
            width=600,
        )

        plots_dict[col_] = (freq_plot + summary_table).cols(1)

    return plots_dict


def density_plots(data, cols, sample_size=10000000):
    ...
    cols: list
        default: empty, takes the requested columns in the given dataframe.
    sample_size: int
        default: 10,000,000, maximum number of data points to be sampled for density plot.
    ...
    if sample_size > 10000000:
        ...
        sample_fraction = min(sample_size / data.count(), 1.0)
        sample_data = data.sample(
            withReplacement=False, fraction=sample_fraction, seed=42
        )

    for col_ in sorted(cols):
        series = sample_data.select(col_)
...
def non_numeric_frequency_plots_v2(data, cols=[]):
    ...
    for col_ in sorted(cols):
        ...
        summary_df.drop("summary", inplace=True)
        summary_df.insert(0, "column", col_)
        summary_table = hvPlot(summary_df).table(
            columns=list(summary_df.columns), height=60, width=600
        )
        ...
        # Append Plot and Table to the list
        plots_and_tables.extend([freq_plot, summary_table])

    # Arrange Plots and Tables in a single layout
    plots = hv.Layout(plots_and_tables).cols(1)

    return plots


def density_plots_v2(data, cols, sample_size=10000000):
    ...
    series = sample_data.select(cols)
    summary_df = series.describe().toPandas().T.round(2)
    summary_df.columns = list(
        np.concatenate(summary_df.loc[summary_df.index == "summary"].values)
    )
...
def feature_density_plots(data, num_cols=[], cat_cols=[]):
    ...
    num_cols : list of str, optional
        List of numerical columns to include in density plots. If not provided, all numerical columns will be used.
    cat_cols : list of str, optional
        List of categorical columns to include in bar plots. If not provided, all categorical columns will be used.

    Returns
    -------
    numerical_plots : dict
        Dictionary containing the density plots of numerical variables.
    categorical_plots : dict
        Dictionary containing the bar plots of categorical variables.
...
def cat_vs_num_bivariate(data, cat_col: str, num_col: str, return_plots=False):
    ...
    if return_plots:
        ...
        if list(pandas_df.columns).count("count") > 1:
            pandas_df.columns = [
                f"count_{12345}" if col == "count" and i == 1 else col
...
def feature_interactions(data, save_as=None, save_path=""):
    ...
    feature_interactions_report["bivariate_plots (Top 10 correlations)"] = (
        bivariate_plots
    )
...
def feature_analysis_pca(data, target_var):
    ...
    model_data = assembler.transform(data)
    pca = PCAml(
        k=2, inputCol=outputCol_name, outputCol=custom_column_name("pca", data.columns)
    )
