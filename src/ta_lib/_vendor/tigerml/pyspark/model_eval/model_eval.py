...
def exp_var(spark, data, y_col, y_hat_col):
    ...
    spark: SparkSession
        Spark session object
    data: pyspark.sql.DataFrame
        Spark dataframe containing the data of actual and predicted target values
    y_col: str
        Actual target column name
    y_hat_col: str
        Predicted target column name

    Returns
    -------
    exp_var: float
        A scalar numeric value calculated as
        Explained Variance = 1 - (variance(y_col - y_hat_col) / variance(y_col))
    """
    un_exp_var = (
        data.groupby()
        .agg(
    ...
    if un_exp_var is None:
        un_exp_var = 0
    exp_var = 1 - un_exp_var
    return exp_var
...
def get_binary_classification_metrics(
    ...
    data : pyspark.sql.DataFrame
        Dataframe with target and predicted probabilities.
    y_col : str
        Column name of actual/target in the data.
    probability_col : str
        Column name containing predicted probabilities.
    y_pred_col : str
        Column name containing predicted labels.
    threshold : float, optional
        Threshold in the range of [0, 1] to consider for the calculation of metrics. Default is 0.5.
    sig : int, optional
        Significance in terms of decimals for metrics as well as the threshold. Default is 2.
    data_type : str, optional
        Type of data, e.g., 'Train', 'Test', etc. Default is 'Train'.

    Returns
    -------
    metrics : pd.DataFrame
        DataFrame containing different classification metric values for the given data.
    """
    relevant_cols = [
...
def get_binary_classification_plots(
    ...
    spark : SparkSession
        The Spark session object.
    data : pyspark.sql.DataFrame
        Dataframe containing actual and predicted probabilities and/or predicted classes.
    y_col : str
        Column name of actuals in the data.
    y_pred_col : str
        Column name of prediction label in the data.
    probability_col : str
        Column name of probability column in the data.
    threshold : float, optional
        Threshold to define the class for calculation of metrics. Default is 0.5.
    feature_cols : list of str, optional
        Columns considered for feature level comparison of the performance. Must be present in the data.

    Returns
    -------
    plots_dict : dict
        Dictionary containing different classification plots.
    """
    conf_matrix, auROC, auPR = get_classification_scores(
        data, y_col, probability_col, y_pred_col, threshold=threshold
    )
