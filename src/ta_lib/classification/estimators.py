...
# def _drop_collinear_columns(X, column_names=None, threshold=1e-12):
#     """Drops linearly dependent columns from X using SVD rank estimation.
...
#         Optional column names, used if X is a NumPy array.
#     threshold : float
#         Threshold for determining singular values.

#     Returns
#     -------
#     pd.DataFrame
#         Cleaned DataFrame with non-collinear columns.
#     """
...
#         # Drop the column with the highest correlation (crude but works)
#         corr_matrix = X.corr().abs()
#         upper_tri = corr_matrix.where(
#             np.triu(np.ones(corr_matrix.shape), k=1).astype(bool)
#         )
#         to_drop = upper_tri.stack().idxmax()[1]
#         X = X.drop(columns=to_drop)

#     return X


class SKLStatsmodelLogit(ClassifierMixin, BaseEstimator):
    """sklearn style wrapper estimator for the statsmodels.api.OLS Estimator."""

    def __init__(self, fit_intercept=True, method="lbfgs"):
    ...
    def __sklearn_tags__(self):
        """Return the scikit-learn tags for this estimator.

        This method provides metadata about the estimator, including
        compatibility with scikit-learn features.
        """
    ...
    def fit(self, X, y, column_names=None):
        ...
        """
        #  all the changes made in this commit, should requrie vigrous testing
        # temp_X, tempy = check_X_y(X, y)

        if isinstance(X, (pd.DataFrame)) and column_names is None:
            column_names = X.columns.to_list()

        check_classification_targets(y)
        X, y = self._validate_data(X, y, reset=True, ensure_2d=True, dtype="numeric")
        ...
        if self.fit_intercept:
            X = sm.add_constant(X, has_constant="add")  # skip
        ...
        if X.shape[0] < 2:
            raise ValueError("Cannot fit model with only 1 sample (n_samples = 1).")
        ...
        # X.columns = X.columns.astype(str)
        # X, y = self._validate_data(X, y, reset=True, ensure_2d=True, dtype=None)
        # X = pd.DataFrame(X)
        # X.columns = column_names
        ...
        try:
            ...
            raise ValueError(
                "Singular matrix: features may be collinear or too few samples"
            ) from e
    ...
    def predict(self, X, threshold=0.5):
        ...
        if X.shape[1] != self.n_features_in_:
            raise ValueError(
                f"X has {X.shape[1]} features, but estimator was fitted with {self.n_features_in_} features."
            )
    ...
    def get_params(self, deep=False):
        ...
        """
        return {"fit_intercept": self.fit_intercept, "method": self.method}
