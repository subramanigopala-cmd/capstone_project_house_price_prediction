...
.. note::
    ...
    * If you already have `Anaconda` installed, please still go ahead and install Miniconda or Miniforge and use it for development.
    * Miniconda/Miniforge provides a lightweight conda environment and is recommended for certain development scenarios.
    * If ``conda`` command is not in your path, you can configure your shell by running ``conda init``
    
.. warning::

    **Anaconda/Miniconda Alternative**: Consider using :ref:`Miniforge <miniforge_installation-ref>` as an open source alternative to Anaconda/Miniconda. 
    Due to Anaconda licensing policy change, installation of packages from ``anaconda`` channel requires commercial license. To avoid that, we recommend 
    installing packages from ``conda-forge`` using ``--channel conda-forge`` in all ``conda`` install/update command. `Miniforge` comes with ``conda-forge`` 
    as default channel. Hence, if you are setting up for the first time or want to avoid modifying all your ``conda`` commands, using `Miniforge` is recommended.
