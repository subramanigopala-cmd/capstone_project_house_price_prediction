...
def setup_env_legacy(c, platform=PLATFORM, env=DEV_ENV, force=False):
    ...
    with py_env(c, env_name):
        ...
        c.run(f"""python -m pip install -e "{HERE}" --no-deps --no-use-pep517""")
...
def _setup_env_common(c, env_name, platform=PLATFORM, env=DEV_ENV, force=False, python_version="3.12"):
    """
...
@task(
    help={
        ...
        "env": "Specifies the environment type. Must be one of ``{dev|test|run}``",
        "force": "If ``True``, any pre-existing environment with the same name will be overwritten",
        "python_version": "Specifies the python version. Must be one of ``{3.12|3.11|3.10}``",
    }
)
def setup_env(c, platform=PLATFORM, env=DEV_ENV, force=False, usecase=None, python_version="3.12"):
    """Help in setup of a new development environment.
...
def setup_env_pyspark(c, platform=PLATFORM, env=DEV_ENV, force=False, python_version="3.12"):
...
def format_code(c, platform=PLATFORM, env=DEV_ENV, path="src/ta_lib"):
    env_name = _get_env_name(env)
    with py_env(c, env_name):
...
def setup_ci_env(c, platform=PLATFORM, force=False):
    ...
    with py_env(c, env_name):
        ...
        # install the current package
        c.run(f"""python -m pip install -e "{HERE}" --no-deps --no-use-pep517""") # To ignore pyproject
...
def run_qc_test(c, platform=PLATFORM, env=DEV_ENV, fail=False):
    ...
    with py_env(c, env_name):
        ...
        c.run(f"""python -m isort --check-only "{SOURCE_FOLDER}" """, warn=(not fail))
...
def start_ipython_shell(c, platform=PLATFORM, env=DEV_ENV):
    ...
    with py_env(c, env_name):
        c.run(f"""ipython --InteractiveShell.simple_prompt=False -i "{startup_script}" """, pty=True)
